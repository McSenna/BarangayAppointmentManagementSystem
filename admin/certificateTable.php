...
try {
    ...
    $sql_residencey = "
        ...
        LEFT JOIN residence_information 
    ...
    if (isset($_REQUEST['search']['value'])) {
        $sql_residencey .= " AND (certificate_request.residence_id LIKE '%" . $_REQUEST['search']['value'] . "%' 
                         OR last_name LIKE '%" . $_REQUEST['search']['value'] . "%' 
                         OR purpose LIKE '%" . $_REQUEST['search']['value'] . "%'
                         OR certificate_type LIKE '%" . $_REQUEST['search']['value'] . "%'
                         OR first_name LIKE '%" . $_REQUEST['search']['value'] . "%')";
    }
    ...
    if (isset($_REQUEST['order'])) {
        ...
        $sql_residencey .= ' ORDER BY id DESC ';
    }
    ...
    while ($row_residency = $query_residency->fetch_assoc()) {
        ...
        if ($row_residency['status'] == 'PENDING') {
            ...
            $tools = '<i style="cursor:pointer;color:yellow;text-shadow:-1px 0 black,0 1px black,1px 0 black,0 -1px black;"
                        class="fas fa-eye text-lg px-2 acceptStatus"
                        id="'.$row_residency['residence_id'].'"
                        data-id="'.$row_residency['id'].'"
                        data-toggle="tooltip"
                        title="View Request"></i>';
            ...
            if ($row_residency['date_expired'] < $date_today) {
                ...
                $tools = '<i style="cursor:pointer;color:red;text-shadow:-1px 0 black,0 1px black,1px 0 black,0 -1px black;"
                          class="fas fa-times-circle text-lg px-2"
                          data-id="'.$row_residency['id'].'"
                          title="Expired"></i>';

            } else {

                $tools = '<a href="printRequest.php?request='.$row_residency['residence_id'].'&purpose='.$row_residency['id'].'"
                           target="_blank"
                           style="cursor:pointer;color:pink;text-shadow:-1px 0 black,0 1px black,1px 0 black,0 -1px black;"
                           class="fas fa-print text-lg px-2"
                           title="Print"></a>

                           <i style="cursor:pointer;color:lime;text-shadow:-1px 0 black,0 1px black,1px 0 black,0 -1px black;"
                              class="fas fa-check text-lg px-2 acceptStatus"
                              id="'.$row_residency['residence_id'].'"
                              data-id="'.$row_residency['id'].'"
                              title="View Record"></i>';
            }
            ...
            $tools = '<i style="cursor:pointer;color:red;text-shadow:-1px 0 black,0 1px black,1px 0 black,0 -1px black;"
                      class="fas fa-times text-lg px-2 acceptStatus"
                      id="'.$row_residency['residence_id'].'"
                      data-id="'.$row_residency['id'].'"
                      title="View Record"></i>';
        }
        ...
        // Format dates
        $date_issued = $row_residency['date_issued'] != '' ? date("m/d/Y", strtotime($row_residency['date_issued'])) : '';
        $date_expired = $row_residency['date_expired'] != '' ? date("m/d/Y", strtotime($row_residency['date_expired'])) : '';

        // BUILD ROW
        $subdata = [];
        $subdata[] = $row_residency['residence_id'];
        $subdata[] = $row_residency['first_name'] . ' ' . $row_residency['last_name'];
        $subdata[] = $row_residency['certificate_type']; // ★ NEW COLUMN ★
